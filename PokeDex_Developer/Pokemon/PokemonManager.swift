...
class PokemonManager:ObservableObject,Pokemon{
    ...
    func getForms(name:String) async throws -> ([String],[String]){
        ...
        var formInfo:([String],[String]) = ([],[])  //폼 이름, 이미지 경로
        for form in forms{
            ...
            if let name = pokemonForm.formNames?.first(where: {$0.language?.name == "ko"})?.name{   //한글 폼이름이 있는 경우
                formInfo.0.append(name)
            }
            else if let name = pokemonForm.formName{       //한글 폼이름이 없는 경우(영문은 존재)
                formInfo.0.append(FormsFilter(rawValue: name)?.translatedName ?? "\(name)")
            }else{
            ...
            if isDefault{
                if let name = pokemonForm.formName{
                    if name.isEmpty{
                        formInfo.1.append("https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/home/\(id).png")  //어떠한 모습,폼도 존재하지 않을때
                    }else{
                        formInfo.1.append("https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/home/\(id)-\(name).png")  //다른 모습이 존재할때
                    }
                }
            }else{
                formInfo.1.append("https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/home/\(id).png")  //다른 폼이 존재할 때
            }
        }
        return formInfo
    }
